    ...
    await new Promise((next) => {
      request(options, async function (error, response) {
        ...
        for (let u = 0; u < JSON.parse(response.body).results; u++) {
          ...
          let status = JSON.parse(response.body).response[u].fixture.status
            .long;
          let timestamp = JSON.parse(response.body).response[u].fixture
            .timestamp;
          let idSub = JSON.parse(response.body).response[u].fixture.id;
          let stadium = JSON.parse(response.body).response[u].fixture.venue
            .name;
